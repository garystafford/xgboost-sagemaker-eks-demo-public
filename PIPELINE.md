...
`pipeline.py` defines and optionally submits an Amazon SageMaker Pipeline that prepares data, tunes an XGBoost regression model with SageMaker Automatic Model Tuning, evaluates the best candidate, explains it with SageMaker Clarify SHAP analysis, records the promoted model in SageMaker managed MLflow Experiments, and registers the resulting model package for downstream deployment. It is the MLOps half of this repository's end-to-end workflow: the model package it registers can later be approved and picked up by the EventBridge/CodeBuild/EKS deployment path described in `README.md`.
...
| 2     | `TuneXGBoost`                | `TuningStep`     | Runs SageMaker Automatic Model Tuning and ranks candidates by validation RMSE (Root Mean Square Error).                                                                                 |
