...
`pipeline.py` defines and optionally submits an [Amazon SageMaker Pipeline](https://aws.amazon.com/sagemaker/ai/pipelines/), a serverless workflow orchestration service purpose-built for MLOps. The pipeline prepares data, tunes an XGBoost regression model with [SageMaker Automatic Model Tuning (AMT)](https://docs.aws.amazon.com/sagemaker/latest/dg/automatic-model-tuning.html), evaluates the best candidate, explains it with [SageMaker Clarify](https://aws.amazon.com/sagemaker/ai/clarify/) SHAP analysis, records the promoted model in SageMaker managed MLflow Experiments, and registers the resulting model package for downstream deployment. It is the MLOps half of this repository's end-to-end workflow: the model package it registers can later be approved and picked up by the EventBridge/CodeBuild/EKS deployment path described in `README.md`.

## High-Level Flow

The pipeline contains six ordered SageMaker steps:
...
| 2     | `TuneXGBoost`                | `TuningStep`     | Runs SageMaker Automatic Model Tuning (AMT) and ranks candidates by validation RMSE (Root Mean Square Error).                                                                                 |
...
| 4     | `RunClarifyExplainability`   | `ProcessingStep` | Launches SageMaker Clarify SHAP (SHapley Additive exPlanations) explainability for the best HPO (Hyperparameter Optimization)  model (Note, Clarify is in a deprecation phase).                                                                                         |
| 5     | `LogPromotedModelExperiment` | `ProcessingStep` | Records the promoted model artifact, Clarify report, tuning job, best training job, and metrics in SageMaker managed MLflow, S3, and classic Experiments APIs. |
| 6     | `RegisterModel`              | `ProcessingStep` | Creates the SageMaker Model Package Group if needed and registers the best HPO model package with quality and explainability metadata.                         |

The pipeline name is `xgboost-eks-pipeline-v3-processing`.

## Processing Scripts

`pipeline.py` references five checked-in processing scripts from the local `code/` directory:
...
| `code/run_clarify.py`    | `RunClarifyExplainability`   | Creates a temporary SageMaker model, launches a SageMaker Clarify processing job, waits for it to finish, and writes a Clarify summary.                                                                |
...
| `code/register_model.py` | `RegisterModel`              | Ensures the model package group exists and calls `create_model_package` with model artifact, inference image, content types, evaluation metrics, and explainability metadata.                          |
...
It retrieves three managed container images:

| Image        | Version            | Use                                                                                     |
| ------------ | ------------------ | --------------------------------------------------------------------------------------- |
| XGBoost      | `1.7-1` / Python 3 | Model tuning candidate jobs and model evaluation.                                       |
| scikit-learn | `1.2-1` / Python 3 | Preprocessing, Clarify launcher, Experiments logging, and registration processing jobs. |
| Clarify      | `1.0`              | SageMaker Clarify SHAP explainability processing job.                                   |
...
- Writes a headered analysis dataset, feature header metadata, and a median feature baseline for SageMaker Clarify.
...
| Analysis   | `/opt/ml/processing/analysis/`           | `s3://<default-bucket>/xgboost-eks-pipeline/preprocessed/analysis`   |

The train, validation, and test split files are written without headers or indexes for the built-in XGBoost container. The analysis output includes `data.csv` with headers, `headers.json`, and `baseline.csv` for Clarify.
...
The tuning objective is `validation:rmse`, minimized with Bayesian search and automatic early stopping. The best model artifact is resolved through `tuning_step.get_top_model_s3_uri(top_k=0, ...)` and reused by evaluation, Clarify explainability, Experiments logging, and registration.
...
### 4. `RunClarifyExplainability`

The Clarify launcher job runs `code/run_clarify.py` in the scikit-learn processing image on one `ml.m5.large` processing instance. That launcher creates a temporary SageMaker model from the best HPO model artifact, starts an official SageMaker Clarify processing job, waits for it to finish, and deletes the temporary model.

The Clarify job uses the headered analysis dataset from preprocessing and runs SHAP explainability with:

- The median feature vector from the training split as the SHAP baseline.
- `num_samples` set to `100`.
- `agg_method` set to `mean_abs`.
- `save_local_shap_values` enabled.

Clarify outputs are written under `s3://<default-bucket>/xgboost-eks-pipeline/clarify/<trial-name>/analysis/`. The expected model registry explainability report URI is `.../analysis/analysis.json`. The launcher also writes `clarify_summary.json` under `.../summary/`.

Because the launcher creates SageMaker resources from inside a processing job, the SageMaker execution role needs permission to create/delete SageMaker models, create/describe processing jobs, pass itself to SageMaker, read model/data artifacts from S3, and write Clarify outputs to S3.

### 5. `LogPromotedModelExperiment`
...
- The best model artifact S3 URI, evaluation report S3 URI, and Clarify report S3 URI as tags.
...
- The Clarify explainability report S3 URI.
- The tuning job name.
...
3. Calls `create_model_package` with:
   ...
   - The Clarify `analysis.json` S3 URI as Model Registry Explainability metadata.
...
- Model registration is unconditional once evaluation and Clarify complete. Metrics are recorded, but there is no active RMSE or R2 threshold.
